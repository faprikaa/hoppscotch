...
export class TeamRequestResolver {
  ...
  async createRequestInCollection(
    ...
    // Type guard to ensure left value is string
    if (E.isLeft(teamRequest)) {
      const error = teamRequest.left;
      if (typeof error === 'string') {
        throwErr(error);
      }
      throwErr('Unknown error occurred');
    }
  ...
  async updateLookUpRequestOrder(
    ...
    if (E.isLeft(teamRequest)) throwErr(String(teamRequest.left));
    return true;
  }

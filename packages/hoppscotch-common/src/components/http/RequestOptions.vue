<template>
  ...
  >
    ...
    <HoppSmartTab
      v-if="properties?.includes('description') ?? true"
      :id="'description'"
      :label="`${t('tab.description')}`"
      :indicator="isDescriptionFilled"
    >
      <HttpDescription v-model="request.description" />
    </HoppSmartTab>
    <HoppSmartTab
...
import HttpDescription from "./HttpDescription.vue"
...
const isDescriptionFilled = computed(() => {
  console.log(request.value.description && request.value.description.length > 0)
  return Boolean(request.value.description && request.value.description.length > 0)
})

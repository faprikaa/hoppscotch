<template>
  ...
  >
    ...
    <CollectionsMyCollections
      ...
      @export-swagger-ui="exportSwaggerUI"
      @export-markdown="exportMarkdown"
      @export-custom-html="exportCustomHTML"
      @export-custom-html-sidebar="exportCustomHTMLSidebar"
      @export-docusaurus="exportDocusaurus"
...
import ExportSwaggerUI from "~/helpers/import-export/export/swagger-ui"
import ExportCustomHTML from "~/helpers/import-export/export/custom-html"
import { ExportCustomHTMLSidebar } from "~/helpers/import-export/export/custom-html-sidebar"
import ExportOpenAPI from "~/helpers/import-export/export/openapi"
import { ExportMarkdown } from "~/helpers/import-export/export/markdown"
import { ExportDocusaurus } from "~/helpers/import-export/export/docusaurus"
...
const exportOpenAPI = async (collection: HoppCollection) => {
  if (collectionsType.value.type === "my-collections") {
    const exporter = new ExportOpenAPI(collection)
    const openAPIObject = exporter.generateOpenAPIObject()
    
    // Convert to JSON string with pretty formatting
    const openAPIString = JSON.stringify(openAPIObject, null, 2)
    
    const name = collection.name
    
    // Use the same download function as exportData for consistency
    await initializeDownloadCollection(openAPIString, `${name}-openapi`)
  } else {
    return
  }
}

const exportSwaggerUI = async (collection: HoppCollection) => {
  if (collectionsType.value.type === "my-collections") {
    const swaggerExporter = new ExportSwaggerUI(collection)
    const swaggerHTML = swaggerExporter.generateSwaggerUIHTML()
    ...
    const result = await platform.kernelIO.saveFileWithDialog({
      ...
      suggestedFilename: `${name}-swagger-ui.html`,
      filters: [
        {
          name: "HTML file",
          extensions: ["html"],
...
const exportCustomHTML = async (collection: HoppCollection) => {
  if (collectionsType.value.type === "my-collections") {
    const customHTMLExporter = new ExportCustomHTML(collection)
    const customHTML = customHTMLExporter.generateCustomHTML()
    ...
    const result = await platform.kernelIO.saveFileWithDialog({
      data: customHTML,
      contentType: "text/html",
      suggestedFilename: `${name}-docs.html`,
      filters: [
        {
          name: "HTML file",
          extensions: ["html"],
...
const exportCustomHTMLSidebar = async (collection: HoppCollection) => {
  if (collectionsType.value.type === "my-collections") {
    const customHTMLExporter = new ExportCustomHTMLSidebar(collection)
    const customHTML = customHTMLExporter.generateCustomHTML()
    ...
    const result = await platform.kernelIO.saveFileWithDialog({
      data: customHTML,
      contentType: "text/html",
      suggestedFilename: `${name}-docs-sidebar.html`,
      filters: [
        {
          name: "HTML file",
          extensions: ["html"],
...
// Add this function after the other export functions
const exportMarkdown = async (collection: HoppCollection | TeamCollection) => {
  if (collectionsType.value.type === "my-collections") {
    exportLoading.value = true
    
    const collectionToExport = collection as HoppCollection
    const markdownExporter = new ExportMarkdown(collectionToExport)
    
    const result = await markdownExporter.download()
...
const exportDocusaurus = async (collection: HoppCollection | TeamCollection) => {
  if (collectionsType.value.type === "my-collections") {
    exportLoading.value = true
    
    const collectionToExport = collection as HoppCollection
    const docusaurusExporter = new ExportDocusaurus(collectionToExport)
    
    const result = await docusaurusExporter.download()
